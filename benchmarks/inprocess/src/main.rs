...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "{}",
        serde_json::json!({"engine": args[1], "warmup_ms": warmup_ms, "samples_ms": samples, "output_bytes": expected.len(), "libclang": if args[1]=="bindgen" {Some(bindgen::clang_version().full)} else {None}})
    );
    Ok(())
}
