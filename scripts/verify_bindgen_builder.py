...
def verify(
    ...
    evidence = {
        ...
        "commands": commands,
        "profiles": {},
        "execution": "configured runner" if runner else "native",
        "runner": runner,
    }
    ...
    def run(command: list[str], name: str, artifacts: Path | None = None) -> str:
        ...
        runner_variable = "CARGO_TARGET_" + target.upper().replace("-", "_") + "_RUNNER"
        if runner:
            environment[runner_variable] = runner
        elif environment.get(runner_variable):
            raise RuntimeError(
                "supply --runner explicitly when using a Cargo target runner"
            )
    ...
    try:
        ...
        evidence["rust_host"] = host
        if target != host and target not in {
            "x86_64-unknown-linux-musl",
            "aarch64-unknown-linux-musl",
        }:
            raise RuntimeError(
                "cross-target consumer validation currently supports musl only"
            )
        if not runner and (
            target.split("-")[0] != host.split("-")[0]
            or ("linux" in target) != ("linux" in host)
        ):
            raise RuntimeError(
                "a foreign CPU or operating system requires an explicit --runner"
            )
        ...
        for path in sorted(
            (target_directory / target / "release/deps").glob("zstd_sys-*.d")
        ):
            ...
            for binding in bindings:
                assert (
                    binding.is_file()
                    and f"Generated by Toucan for {target}." in binding.read_text()
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "--runner", default="", help="Explicit Cargo runner for a foreign CPU"
    )
    ...
    result = verify(cache, output, args.target, profiles, args.runner)
