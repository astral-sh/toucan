...
pub(crate) fn compute_layout(
    ...
    super::compute_layout(target, compiler, ty, Dialect::Mingw)
}
...
fn layout_field(rlb: &mut RecordLayoutBuilder, field: &RecordField<()>) -> Result<()> {
    let ty = compute_layout(rlb.target, rlb.compiler, &field.ty)?;
    ...
    // See test case 0069.
    let update_record_alignment = field.bit_width.is_none()
