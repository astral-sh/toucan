...
use crate::target::{Compiler, Target};
use crate::util::{
...
fn compute_layout(
    ...
    match &ty.variant {
        ...
        TypeVariant::Record(r) => compute_record_layout(
            dialect,
            target,
            compiler,
            r.kind,
        ...
        TypeVariant::Enum(v) => compute_enum_layout(target, compiler, v, &ty.annotations),
        TypeVariant::Typedef(dst) => {
            // #pragma pack and __attribute__((packed)) are ignored on typedefs.
            // See test case 0088.
            let dst_ty = compute_layout(target, compiler, dst, dialect)?;
        ...
        TypeVariant::Array(a) => {
            let ety = compute_layout(target, compiler, &a.element_type, dialect)?;
            Ok(Type {
                layout: TypeLayout {
...
fn compute_enum_layout(
    ...
    let max_alignment = match compiler {
