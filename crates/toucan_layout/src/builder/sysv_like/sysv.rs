...
pub(crate) fn compute_layout(
    ...
    super::compute_layout(target, compiler, ty, Dialect::Sysv)
}
...
fn layout_field(rlb: &mut RecordLayoutBuilder, field: &RecordField<()>) -> Result<()> {
    let ty = super::compute_layout(rlb.target, rlb.compiler, &field.ty, Dialect::Sysv)?;
...
fn layout_bit_field(
    ...
    if width == 0 {
    ...
    } else if rlb.compiler == Compiler::Gcc {
        ...
        assert_eq!(rlb.compiler, Compiler::Clang);
