...
unsafe extern "C" fn stress_i8(x:i8)->i8 {if x<0 {x+1} else {x-1}}
unsafe extern "C" fn stress_i16(x:i16)->i16 {if x<0 {x+1} else {x-1}}
unsafe extern "C" fn stress_u8(x:u8)->u8 {if x==0 {255} else {x-1}}
unsafe extern "C" fn stress_u16(x:u16)->u16 {if x==0 {65535} else {x-1}}
...
fn main(){unsafe{
    ...
    #[cfg(toucan_atomic_narrow_calls)] {
        ...
        assert_eq!(c_narrow_stress(Some(stress_i8),Some(stress_i16),Some(stress_u8),Some(stress_u16)),0);
    }
