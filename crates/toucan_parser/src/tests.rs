...
#[test]
fn failed_function_definitions_release_parameter_scopes() {
    for source in &[
        "int f(int T,",
        "int f(int T)",
        "int f(int T) {",
        "int (*f(int T))(int U) {",
        "int f(int (*callback)(int T)) {",
    ] {
        let mut env = Env::with_gnu();
        env.add_typename("T");
        env.add_typename("U");
        assert!(parser::translation_unit(source, &mut env).is_err());
        assert_eq!(env.symbols.len(), 1, "{}", source);
        assert!(env.is_typename("T"), "{}", source);
        assert!(env.is_typename("U"), "{}", source);
        // Exercise another parse in the same environment, including the normal
        // declaration branch and the identifier-list fallback in declarators.
        parser::translation_unit(
            "int proto(int T); int (*other(void))(int U); int old_style(a); T value; U other_value; int defined(int T) { return T; } T after;",
            &mut env,
        ).unwrap();
