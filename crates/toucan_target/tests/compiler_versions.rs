...
#[test]
#[ignore = "requires Clang with the ARMv7 hard-float backend"]
fn armv7_hard_float_predefines_match_cross_target_clang() {
    use std::process::{Command, Stdio};

    let target = Target::Armv7UnknownLinuxGnueabihf;
    let model = CompilerProfile::default_for(target).predefined_macros();
    let output = Command::new("clang")
        .arg(format!("--target={target}"))
        .args(["-std=gnu11", "-dM", "-E", "-x", "c", "-"])
        .stdin(Stdio::null())
        .output()
        .expect("cross-target Clang must be installed");
    assert!(
    ...
    assert!(!native.contains_key("__SIZEOF_INT128__"));
    assert!(!model.contains_key("__SIZEOF_INT128__"));
}
