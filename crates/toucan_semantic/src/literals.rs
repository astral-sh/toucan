...
/// lang-c omits universal character names from its character-constant grammar.
/// Present each such escape as a hexadecimal escape of the same byte length to
/// the parser, retaining the original token for semantic decoding at its span.
/// The input has already had comments removed, without changing byte offsets.
pub(crate) fn normalize_character_escapes(
    source: String,
) -> (String, std::collections::HashMap<usize, String>) {
    if !source.contains("\\u") && !source.contains("\\U") {
        return (source, std::collections::HashMap::new());
    }
    ...
    while index < bytes.len() {
        ...
        while index < bytes.len() && bytes[index] != quote {
            if bytes[index] == b'\\' {
                index += 1;
                if quote == b'\'' && matches!(bytes.get(index), Some(b'u' | b'U')) {
                    escaped.push(index);
                }
            }
            index += 1;
        }
        if !escaped.is_empty() && index < bytes.len() {
            originals.insert(
                start,
                String::from_utf8(bytes[start..=index].to_vec()).expect("source was UTF-8"),
            );
